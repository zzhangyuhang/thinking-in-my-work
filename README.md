...
## <font color="Coral">4.23,拉链表</font>
### 概念
* 拉链表是什么?
	* 同时维护历史状态和最新状态数据的一种表
* 拉链表特点
	* 拉链表根据拉链粒度的不同，对变化的数据(依粒度的最终状态)进行添加,同时保留历史状态.
* 与其他表区别
	* 全量周期快照表与拉链表
		* 快照表根据定义的快照周期对数据进行全量的快照,如果变化的数据范围很小,则存在大量不变的冗余数据.
		* 拉链表相当于快照做了优化，去除了没有发生变化的数据,仅仅记录变化的数据.
	* 流水表与拉链表
		* 流水表对于表的每一个修改都会记录，可以用于反映实际记录的变更,但效率低
		* 拉链表根据拉链粒度,来进行快照,并不是每一个记录都会记录,而是记录拉链粒度的最终状态.相对于流水表,拉链表中的数据精简,效率高,也能很大程度反映实际记录的变更.

* 拉链表优点
	* 反映历史状态变化
	* 仅记录变化的数据,数据精简
* 拉链表缺点
	* 不能反映所有历史状态变化,反映的范围取决于拉链表的拉链粒度
	* 不能独立完成,对更新的快照表有很大依赖.
	* 所有数据都在一张表中,随着时间增长,数据量巨大,影响性能.
	
* 注意:拉链表的拉链粒度就相当于周期性快照表的周期.

### 应用场景
1. 表中数据量比较大.
2. 表中小部分字段会被update,大部分字段不变.
	* 例如:订单状态.
3. 有查看某个时间点或者时间段历史快照信息的需求.
	* 例如:前天有多少订单未发货.
4. 变化的比例和频率都很小.
	* 例如:1000W订单总量,每天只有10W订单状态需要变更.

* 如果采用周期性快照表来记录数据快照,那么每次全量保存的快照有很多数据都是不变的,是冗余的,十分浪费存储空间.
* 采用拉链表可以对历史状态进行记录,又可以减少不变数据的存储空间.

### 示例
#### 数据库中有一张订单表(订单创建时间,订单编号,订单状态)

1. 在6.20号有3条记录

![0620](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/0620数据.png)

2. 在6.21号有两个新订单添加,一个订单状态发生变化

![0621](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/0621数据.png)

3. 在6.22号有一个新订单添加,两个订单发生变化

![0622](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/0622数据.png)

#### 在数据仓库中如何保存?

1. 周期性(一般以天为单位)的全量覆盖导入数据仓库中
	* 优点
		* 数据量小
		* 当前状态直观
	* 缺点
		* 没有记录历史状态变化,不能满足历史数据的需求分析

2. 周期性(一般以天为单位)的全量添加到数据仓库中,以天为分区
	* 优点
		* 能满足符合快照单位的历史数据的需求分析
	* 缺点
		* 数据冗余严重,极大浪费空间
...
3. 拉链表
	* 和保存周期性快照类似,拉链表只添加修改的数据到数据仓库中.
	* 优点
		* 大大减少了数据冗余
		* 记录了大部分历史信息
	* 缺点
		* 拉链粒度导致一些原子操作没有记录到,不能满足所有的历史数据分析
	* 所以拉链粒度选择的好坏将直接决定拉链表的优劣

#### 拉链表结构

* 每张拉链表最好加入start\_date,end\_date两个字段
	* start\_date:代表状态生命周期的起始时间
	* end\_date:代表状态生命周期结束时间
		* '9999-12-31'表示无结束时间,也就是当前处于有效状态.
* 在每次更新拉链表的时候只用更新两种数据(变化的当前状态数据和新增数据)
	* 非新增且发生状态变更的非历史数据,即end\_date = '9999-12-31'且发生变化的的数据
		1. 更新原有数据的end\_date = today
		2. 添加新的变更记录,start\_date = today,end\_date = '9999-12-31'
	* 新增数据
		1. 直接添加,start\_date = today,end\_date = '9999-12-31'

![拉链表](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/拉链表.png)

* 过程示例
	1. 6.20-6.21,新增了两个订单004,005.001状态发生变化
		* 新增订单(004,005)
			* start\_date = today,end\_date='9999-12-31'
		* 变化订单(001)
			* 修改原有数据,update end\_date=today ... where id=001 and end\_date='9999-12-31'
			* 添加新的数据,append (001,支付完成,today,'9999-12-31')
	2. 同理,6.21-6.22
...
* 拉链表在制作过程中需要一些表的协助
	* 订单当前状态表(推荐)
	* 订单更新表(添加update_time和create_time来筛选出增量数据)
	* 订单流水表(需要按照拉链表粒度进行筛选出最终状态)

* 创建拉链表
```sql
--创建订单拉链表
create table if not exists order_his(
	create_date string comment '订单创建日期',
	order_id int comment '订单ID',
	order_status string comment '订单状态',
	start_date string comment '状态生命周期开始日期',
	end_date string comment '状态生命周期结束日期'
)
row format
delimited fields terminated by '\001'
collection items terminated by '\002'
...
from(
	--修改历史数据的有效状态(结束时间)
	select b.create_date,b.order_id,
			b.order_status,b.start_date, --拉链表的原始的订单状态和周期开始日期
		   if(b.end_date = '9999-12-31' and a.order_status != b.order_status,${hiveconf:today},'9999-12-31') --修改有效状态,结束时间为today,否则不变
	from order a  --订单拉链表
	join order_his b --订单表
	on a.order_id = b.order_id 
	
	union all 
	
	--添加修改的数据(原有且修改的数据和新增数据	
	select a.create_date,a.order_id,
		    a.order_status, --订单表新的订单状态
		    ${hiveconf:today} as start_date, --当前时间为周期开始时间
		    '9999-12-31' as end_date
	from order a --订单表
	left join order_his b --订单拉链表
	on a.order_id = b.order_id 
	where b.order_id is null --新增订单
	or b.end_date = '9999-12-31' and a.order_status != b.order_status --修改过的有效订单数据	
) t 
```
...
```sql
	select * 
	from order_his
	where start_date <= '2012-06-21' 
	  and end_date >= '2012-06-21'
```

![拉链表查询结果](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/拉链表查询结果.png)

* 注意:有时候在拉链表中记录变化次数也会有很大用处.

### 如何解决拉链表的问题
* 所有数据都在一张表中,随着数据增长导致查询性能低下
	* 对start\_date或者end\_date做索引
	* 只暴露部分历史数据:内部存储了全量的历史数据的拉链表,但在分析的时候使用最近3个月的部分历史数据

### 思考
* 我现在公司对于订单并没有采用拉链表的形式,而是采用当前快照的形式之上,每天覆盖前一天的订单表,收集状态流水表的变成状态到历史状态的map中.
* 为什么这么做?
	* 首先这么做肯定不能满足类似'查看某一天的快照'这样的需求的.但是我们平时并没有这种需求.如果要是有查看某一天快照这样需求,那么就需要拉链表了.
	* 拉链表也是有拉链粒度的,并不能展示所有的历史状态变更.订单状态变快时间还是比较快的,不能算是缓慢.如果采用拉链表,以天为拉链粒度,不能满足我们对历史状态的需求.如果采用我们这种方式,能收录一个订单的所有状态变更,同时还不用记录多余的其他字段,节省了空间.

	
## <font color="Coral">4.25,数据仓库反作用于APP</font>
### 问题提出
* 从美团过来的新大佬在讲解他们之前使用的BI数据可视化平台的时候,发现一个很明显的功能就是你选择各种维度,然后根据你的选择维度呈现各种指标.然后这些指标的细节数据你是可以在BI平台上操作的.
	* 比如说,当天异常骑手维度,展现出了个数指标.我们点击进去可以对异常骑手发起通知功能.
* 我们现在的BI数据可视化平台就是仅仅对各个维度指标的展示.不但维度选择不自由,还不能反作用于APP.之前有过召回短信的例子就是,我们人工筛选流式用户,然后总结出名单给运营.运营在发送数据到流式用户.是个分离的过程,而且全程人工化.
* 用户画像方面我们公司还没有做,而且很重要.

### 思考
* 数据平台要反作用于APP
* 用户画像很重要

## <font color="Coral">8.7,关于缓慢维度变化问题</font>
### 问题提出
* 今天收到一个数据更新定义回刷的需求,在进行回刷的时候是根据用户登录地来进行筛选的.但是用户登录地是变化,由于维度表并没有处理缓慢维度变化历史,直接采用的的覆盖导致历史数据回刷不可能准确,因为是根据登录地来的暑假效应对指标影响很大,会造成很大的误差.

### 如何解决
1. 采用当日日活表来计算的话,虽然会减少变化误差.但是还是存在些许误差.
2. 采用维度快照表,其实维度快照表是解决缓慢维度变化问题最简单的方法,但是有可能及其浪费存储空间.如果是用户维度表很大且基本无变化的情况下,每天都要重复保存大量数据.不划算
3. 采用拉链表,拉链表是维度模型解决缓慢维度变化问题的最有效的方法.但是在本问题中用户登录地在每次刷新表的时候最少每个用户会产生一条数据,这样做的话未免跟日活表是一样的.还会导致在对客户维度进行信息筛查的时候加大性能消耗.

### 思考
* 其实问题所在并不是无法维护历史数据,问题真正所在点在于日活表和更新和用户表的更新存在不同步导致的延迟误差.如果跟美团一样保存着每个维度的每天的快照未免不可以,但是前提你得有空间去存储.
* 最后解决方法:采用日活表重新计算,虽然有误差,但是误差不是很大.经过商量是可以接受的.
* 这个问题出现说明了数据仓库中对于历史数据的维度是很重要的.以及存储缓慢维度变化产生的历史数据同样也是值得思考的一个问题.


## <font color="Coral">8.14,关于分析需求的总结</font>

* 在日常处理需求的时候经常也会被需求方索问怎么回事.例如,某一天日活突然悬崖式下跌,这个时候很有可能收到运营方的索问,这是怎么肥四?
* 从另一方面来讲,如果你经常收到业务方提问数据问题说明了你们的数据平台也是做得不够好的.连最基本分析都完不成就别提能辅助决策了.你同时也会在无穷无尽的临时业务需求中挣扎.再换个角度来说,如果你不会解决这样的业务分析问题,你也不太可能提供出有效的数据.这可以说是相互影响的.总的来说就是,要先了解业务,学会分析业务问题才是做数据的基础,有了良好的基础才能够做出更好的数据平台提供出更好的数据方便需求方分析.方便你我他.
* 回归正题,如何才能快速进行业务数据分析呢?一句话"维度拆解,抽样看明细"!
1. 首先,我们要明确发生的问题是什么?
	* 我们可以先给自己来个哲学三连:是谁?从哪来?到哪去?
		* 是谁 -> 用户日活?谁的日活?
		* 从哪来 -> 平时日活都由什么组成?
		* 到哪去 -> 消失的日活用户去哪了?
	* 有时候哲学三连可能还不能将问题弄清楚,那么我们再来个英文五连:
		* who? -> 日活?谁的日活?
		* when? -> 什么时候下跌上涨了?
		* where? -> 在哪下跌上涨了呢?
		* what? -> 发生了什么呢?
		* how? -> 怎么发生的呢?
	* 其实哲学三连的是谁/从哪来和英文五连的who/when/where是帮助我们真正定位问题所在的.而到哪去和what/how才是我们真正要解决的问题.

2. 其次,以不同维度为主导进行拆解.
	* 注意维度层次之间的对应的关系.有的维度之间层次联系很紧密,有的也很松散.
		* 以用户维度为主导:
			* 新用户下跌?新增渠道出问题了?宣传不够?拉新活动不够刺激?注册流程出问题了?竞品砸钱火了我们不行了?
			* 老用户下跌?APP改版了?是否流失了?流失用户画像?
			* 以用户维度出发,我们通常要关注的是运营活动的发生,这个时候往往日常活动航海日志就起到了关键作用.
		* 以APP维度为主导:
			* 版本更迭出问题了?页面不够美观了?页面不够简洁明了了?流程漏斗阻塞用户了? -> 这些都是直接导致老用户流式的问题所在,跟新用户关系很小.
			* 注册流程阻塞用户了? -> 直接影响新用户注册
			* APP没有什么实质内容?APP使用繁琐?APP核心价值太难以体验?引导不够? -> 这些并不会直接导致新用户日活下降,倒是会影响新用户的留存.
			* 往往APP层面出问题的话,我们需要拆解各个流程找到问题流程所在进行优化.这个时候各个流程/页面的漏斗就有很大用处了.以及每个按键的PV/UV也能看出前后的使用率.
		* 以时间维度为主导:
			* 上周这个时候怎么样?同样下降了?还是上升了?
			* 上个月这个时候怎么样?趋势如何?
			* 去年这个时候呢?
			* 往往通过时间的同比环比,能看出大体趋势是否正常.如果同比环比趋势很都差不多,基本可以定论为正常现象,我们只需要考虑生活中什么规律导致的,暑假效应?春运效应?
		* 有时候我们还要关注站外维度:
			* 整个行业的发展/行业运作规律
			* 竞品APP的发展
			* 同样这样的关注也能帮你找到真正问题的所在

3. 最后,提出自己的假想,数据抽样验证.


* 回想自己之前工作的内容,也经常收到日活趋势变化分析的需求.也总结了几条规律.(分析的需求都是直播相关).
	1. 日活暴跌.要是老用户日活下降明显.这个时候我们优先要考虑APP的变更导致的.事实也是如此,因为直播线路网络问题导致直播时候经常出刺耳杂音.如果没有任何问题,需要考虑流式用户的问题了.也就是通过模型来判别是否是流式用户,然后寻找流式用户画像.要是新增下降一般跟宣传的渠道/增长渠道/拉新活动有关系,一般跟APP变更关系不大.
	2. 日活不正常波动. 有时候某个地区新用户迅猛增长,有时候某个地区老用户下降的厉害.优先查看最核心的部分是否出问题. 因为是直播行业,主要用户流量还是因为TOP级的主播吸引来的. 当有个大主播或者新人主播突然做个活动,真的会使新老用户日活上涨.尤其是新用户. 之前有个禁食徒步者一个一夜爆红的新人主播,走到哪,哪里的新增就猛涨. 当然正常直播(无活动情况)直播观众所在地区跟主播所在地区联系比较浅. 还有就是, 当一个直播被分享次数暴涨的时候,肯定是拉高新增的主要途径最健康最体验app价值的途径. 所以记录分享数据各个维度的信息(一般某个地区观众数上涨了,新增数上涨了,没有任何活动的情况下我都会优先看看该地区直播分享的数据)和引导用户完成分享很有必要.
	3. 日活上涨.这样的需求收到较少.这个时候航海日志能说明大部分问题(没处理过有实质的问题所以只是片面的理解).
	4. 一定要进行同比环比分析!!!不要上来就一通分析.最近遇到一个日活变动的分析需求.需求内容是"一个地区日活暴跌,而另一个地区日活暴涨,为什么?".当时我还没找到问题所在就开始拆维度,拆来拆去发现都比较正常.两个地区上涨下跌也都是很平稳的,也没有什么影响因素,为什么呢两地方的日活就平稳的掉个er了呢? 其实生活中细心点这个问题就很好发现,暑假迁移了.做个去年同比,发现趋势一模一样.基本就可以认定为暑假效应迁移了.同样还有什么春运效应啥的.多多了解生活规律才会更好的发现数据规律.
	5. 一定要了解行业发展规律!!!球鞋市场每到夏季就是淡季,在618的时候疯狂做了活动,但是转眼到了7月,日成交单反而下降得很厉害. 不过球鞋运营都是大佬,他们给了解释这是球鞋市场淡季很正常. 了解行业发展规律真的也很重要!!!!


## <font color="Coral">8.19,关于数据分层</font>
### 问题思考
1. 为什么要数据分层?数据分层有什么作用?数据分层有什么好处?
	1. 清晰的数据结构:每一层数据都有自己的职责和作用,方便理解和使用.每一层都有单一的逻辑方便开发与维护.
	2. 清晰的数据血缘关系:数据分层从上到下都是依赖的关系,当问题出现更好的定位问题所在的范围.
	3. 减少重复开发/数据计算:数据中间层能很好的避免重复开发以及重复的数据计算
	4. 屏蔽跨层次间的影响:例如业务层的业务计算方式变更,但是依赖数据没有发生变化.仅仅改变业务层计算逻辑.业务层以下的层次不用发生变化.

2. 其实企业中会有大量的数据表汇总到数据仓库中,试想一下,如果没有经过仔细的分类,在杂乱无章的海量表堆中找出符合逻辑的字段是多么令人头疼的事情.即便你业务逻辑很棒,但是在计算结果的时候每个指标都要跑很久,也同样是很苦恼的事情.所以我们要对数据分层,对数据进行规则化分类化整理,构造中间层和业务层,减少重复开发和大量的数据计算工作,以空间换取时间.
3. 在我刚开始理解数据分层的理念的时候,曾经任务数据分层就是把数据分开,方便管理和维护(哪里出错就改哪里).其实这么理解是片面的,数据分层的每层之间(从上到下)是具有依赖关系的,并不是简单的分开管理.有时下游层次出现大的问题,上游表也需要进行修复的.理解问题的偏差在于,我的理解是觉得数据分层就是垂直的给数据分开,例如一个业务一个层次,但是实际采用的分层策略是水平分层.

![杂乱无章的表](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/杂乱无章的表.jpg)

> 上面是一对杂乱无章的表堆,层级混乱,我们从中梳理出各个表之间的依赖关系是一件头疼的事情.
> 

![杂乱无章的表](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/分层后的表.jpg)

> 上面是分层后的表,有明确的分层逻辑,各个表之间的依赖关系一目了然.
> 

### 如何分层?怎样分层?
* 数据仓库的四个特点:面向主题/集成的/反映历史的/稳定的.设计分层的时候不能违背数据仓库的四个特点,要完成集成又要有相应主题,还要反映历史.一般数据分层是配合维度模型来共同完成数据仓库建设的.
* 每个公司都有不同的分层以及分层的叫法,但是总体架构和功能是差异不大的.我这里抽象一下.

![抽象的数据分层](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/抽象的数据分层.jpg)

> 上图是抽象后的数据分层.一般数据分层都会被分成数据接入层,数据中间层和数据应用层三层.
> 
> 数据接入层也就是我们平时所说的操作数据层(ODS层),这层中我们一般会保存来自业务库的所有数据表和日志数据.这些数据都是原子粒度数据,在导入过程中仅仅是需要保证数据格式一致,不做任何校验.要求最大力度的保证最全的历史数据.
> 
> 数据中间层就是数据仓库层(DW层),一般数据仓库层我们还会进一步划分为DWD(data warehouse detail)层,DWS(data warehouse summary)层,DWM(data warehouse market)层.在这层中,我们主要采用了维度模型来进行建设.维度模型和数据分层配合完成数据仓库建设就体现在这层.DWD层,数据明细层,是原子粒度的维度模型数据层.在这层中主要是对ODS层的数据进行主题域划分/一致维度事实划分等等维度建模的步骤来搭建原子粒度的维度模型.过程中需要简单的做一些数据去重去脏等工作,还要做一些异常数据的处理,例如刷单数据屏蔽过滤,但是不做任何的汇总操作.DWS是在DWD基础上对原子粒度数据按照不同维度聚合产生的高粒度的数据层级,是按照单一业务来进行划分数据表的,在这层中对一些行为数据进行组装,例如对可累加性指标的汇总,来提升公共指标的复用.DWM则是在DWS的基础上对于一些相关联的单一业务在进行汇总,是跨过业务场景的,可以理解为是对多业务场景基础上再进行抽象汇总,一般DWM层中这些汇总都是以大宽表的形式存在的.
> 
> 最后是数据应用层,一般提供出去的数据都存在于数据应用层,叫法有很多种,有的公司叫APP层有的公司叫ADS层,我们这里叫APP层,直观.例如:可视化平台上使用的数据,报表数据,分析数据等等都是存在于APP层中的.这层数据层主要是对外来提供服务的,在这层主要关注的是减少非数据仓库人员使用数据仓库数据的门槛,减少对数据底层的关注.
> 
> 思考:为什么要有中间层呢?直接从数据接入层到数据应用层不好吗?方便省事.
> 
> 没有数据中间层真的省事情吗?未必,虽然在数据使用逻辑上少了一大块的数据流转,但是过程中暴露的问题又会产生新的麻烦.
> 
> 首先,就是重复计算的问题,例如,应用层不同应用采用一款相同的指标体系,如果没有中间层,我们每次获取的时候就要重复计算一次,浪费计算时间和资源.同时还不能保证数据的实时性,有了中间层保存最终指标,相当于用空间换取了时间.
> 
> 再次,就是方便关系和维护.回到最开始的问题,为什么要数据分层.如果没有中间层,以及中间层的层层分层.那么我们在抽取个ODS和APP层又有什么作用呢?所以中间层正是数据分层的关键点所在,重中之重.
> 
> 最后,就是数据一致性和数据质量问题.如果数据没有一致性保证,那么数据的可信度为0.我们日常要求的指标很多都是有多种计算方式的.各个方式之间难免有计算误差.同一个指标不同结果,所有的结果都是不可信的.当数据不可信那么数据就失去了存在的意义.采取中间层并且在中间层采取了维度一致性模型,保证了数据的一致性和质量问题.你只要计算口径和预订的一样,计算出来的数据肯定是一致的,这也是中间层所必须保证的.

![细化数据分层](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/细化数据分层.jpg)

* 数据分层之间的数据流向

![数据流向](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/数据流向.jpg)

> 上图是各个层次间的数据流向.数据流向都是从下往上的.在流动过程中是不允许逆向调用的,不能用DWS层数据反过来构建DWD层数据的.在APP层可以有多个中间层的数据.因为APP所需要的数据粒度可能不同,可以接收下面层级数据.但是在计算过程中不同层次的数据间不能进行操作.
> 
> 我们在使用数据仓库过程中,也要遵从一定的数据流向.如图,我们在使用数据的时候优先使用公共层(DWM/DWS)的数据,如果不能满足要求的情况下在依次向下调用.在使用过程中要避免跨层次调用,例如在聚合的时候时候不能将DWM层和DWD层间的数据进行汇总.
> 


## <font color="Coral">8.21,数据仓库痛点</font>

![数据仓库痛点以及解决方法](https://github.com/zzhangyuhang/thinking-in-my-work/blob/master/数据仓库痛点.jpg)

> 上图是数据仓库容易出现的痛点.
> 
> 其实每个痛点出现在不同的数据仓库发展的过程.
> 
> 痛点1和痛点9出现,说明你的数据仓库没有做好数据服务化.这时候需要开发统一的数据可视化共享平台/数据自提/数据多维分析系统来更好的提供数据服务.
> 
> 痛点2 痛点3 痛点4 痛点5 出现则说明数据仓库的数据模型设计的不好,甚至可能没有数据模型的支撑.数据仓库采用维度模型来规范数据仓库架构,维度模型能保证数据一致性和规范化.数据模型中采用分层的概念,增加汇总层和数据集市层增加数据的复用性,可以大幅度提高数据产出效率.
> 
> 痛点6出现则说明你的数据仓库迫切的需要构建良好的元数据系统.元数据系统对于整个数据仓库来说很重要,也是向外部人员展示你数据仓库成果的途径.一个良好的元数据系统可以大大减少工作人员对数据仓库的开发和维度时间成本.
> 
> 其实,在做数据仓库的时候,我们工作虽然是产出数据表.但是你的思维不能仅仅限制于你就是个提数机器.做数据的时候要把数据向数据产品化/数据平台化的方向发展.我们产出的数据是为了给需求方提供更好的服务,那么你做的数据才有意义.
> 

## <font color="Coral">9.21,日活和月活之间的关系</font>

* 我们时常在工作中谈到一个app的日活和月活,那么日活和月活之间有什么关系吗?日活和月活字面上大多数人都理解为 日活越高,肯定月活就越高. 这对吗? 
* 这里回答不一定. 其实对于日活来说只是当天活跃的uv,而对于月活来说是一个月里活跃的uv. 日活用户包括:老用户和新增用户. 月活是日活这个月的汇总去重. 我们可以假设一种情景:每天活跃的都是一个月前的老用户,这个月一个新用户增加.那么这个月的月活最大就是一个月前的老用户数. 正常情况是: 每天实际活跃的有新用户和老用户, 昨天活跃的用户在今天算是老用户,前天活跃的用户在今天算是老用户,...,上个月活跃的用户在今天算是老用户.所以老用户的基数就是(某天的新增*某天之后的留存系数)的加和. 新用户呢?就是某天的新用户. 我们再假设日活为一个固定值,就可以将日活提出出来.  如果用日活/月活. 日活就可以消除掉.只剩下留存系数. 我们还知道留存系数是根据时间来说减低的. 所以结论就是  日活与月活之间的关系  与用户量是没有关系的, 只与留存系数和时间有关系. 

* 具体证明:[日活和月活之间的关系](https://coffee.pmcaff.com/article/1732075580352640/pmcaff?utm_source=forum) 

* 结论:到这里我们就可以发现，类似'新老用户比'、'日活月活比'等等数据指标，其实从根本上就只跟留存系数有关，而留存系数，则只跟留存和时间有关。所以，虽然有很多文章会说，这些指标通常用来反应用户使用频率、用户粘性、用户忠诚度等等，让人觉得非常模糊，但现在我们清楚了，说到底还是留存，只要留存好，这些数据都会好.
